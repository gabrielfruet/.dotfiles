...
stop_container() {
    local container
    container=$(sudo docker ps --format '{{.ID}}: {{.Names}}' | fzf --prompt="Select a container to stop: ")
...
start_container() {
    local container
    container=$(sudo docker ps -a --filter "status=exited" --format '{{.ID}}: {{.Names}}' | fzf --prompt="Select a container to start: ")
...
attach_container() {
    local container
    container=$(sudo docker ps --format '{{.ID}}: {{.Names}}' | fzf --prompt="Select a container to attach to: ")
...
# Main menu using fzf
action=$(echo -e "Stop a container\nStart a container\nAttach to a container" | sudo fzf --prompt="Select an action: ")
