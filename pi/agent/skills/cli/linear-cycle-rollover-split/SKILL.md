...
4. State the pair with its dates, render one row per issue, and ask for `X` in a
   ...
   in cycle `D` (each ≤ E)". The user can override the pair in the same reply.
5. Echo the plan back per issue (`#1: 3 done in D, 2 carry to C`) and get a
   confirm before touching anything.
6. For each issue, with `R = E - X`, run the split:
   ```bash
   # done half
